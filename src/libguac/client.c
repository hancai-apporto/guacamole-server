...
int guac_client_end_frame(guac_client* client) {
    return guac_client_end_multiple_frames(client, 0, 0);
}
...
int guac_client_end_multiple_frames(guac_client* client, int frames, int size) {
    ...
    guac_client_log(client, GUAC_LOG_TRACE, "Server completed "
            "frame %" PRIu64 "ms (%i logical frames, %d bytes)", client->last_sent_timestamp, frames, size);

    return guac_protocol_send_sync(client->socket, client->last_sent_timestamp, frames, size);
